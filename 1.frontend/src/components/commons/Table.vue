<template>
  <div :id="id" class="mt-4">
    ...
    <div class="table-responsive">
      <table class="table border table-hover table-striped"  ref="tablaDatos">
        <thead>
          <tr>
            <th
    ...
    <div class="text-center w-100 me-4" v-if="pageJumpOption"><small>{{ jumpPageText }}</small> 
      <input 
        type="number" 
        style="max-width: 50px;" 
        @input="jumpPage($event)"
        :max="totalPages"
        min="0"
      >
    </div>
...
export default {
  ...
  computed: {
    ...
    placeHolderText: function(){
      return this.language == 'EN'? 'Search' : this.language == 'JP'? '検索':'';
    },
    entriesText: function(){
      return this.language == 'EN'? 'Entries per page: ' : this.language == 'JP'? 'ページ当たりの表示数：':'';
    },
    previousText: function(){
      return this.language == 'EN'? 'Previous' : this.language == 'JP'? '前のページ':'';
    },
    PageText: function(){
      return this.language == 'EN'? 'Page' : this.language == 'JP'? 'ページ':'';
    }, 
    nextText: function(){
      return this.language == 'EN'? 'Next' : this.language == 'JP'? '次のページ':'';
    },
    jumpPageText: function(){
      return this.language == 'EN'? 'Jump to page: ' : this.language == 'JP'? 'ページへ移動：':'';
    },
  },
  watch: {
  ...
  methods: {
    ...
    emitCellEvent(key, index){
      this.$emit(`${key}Event`, index)
    }
  },
};
</script>
